// // universalProgramDays.ts
// import { openai } from "./OpenaiClient";

// export type BuildArgs = {
//   // free-form; can be omitted — model will infer from request text
//   plan_type?: string | null; // e.g. "Training", "Sleep", "Nutrition", "Hybrid", ...
//   weeks: number;             // you decide how long
//   request_text: string;      // raw user ask (free text)
//   hints?: {
//     days_per_week?: number | null;   // if user mentioned 3/4/5/etc
//     modalities?: string[] | null;    // if user mentioned "boxing", "BJJ", etc
//     goals?: string[] | null;
//     constraints?: string[] | null;   // injuries, equipment limits, time windows, etc
//   };
// };
...
//         additionalProperties: false,
//         properties: {
//           plan_type: { type: "string" },            // model-inferred if not provided
...
//           additionalProperties: false,
//           properties: {
//             active: { type: "boolean" },            // true if it’s a working day
//             notes: { type: "string" },
//             intensity: { type: ["string","number"] }, // e.g. "easy"/"moderate"/"hard" or 1–10
...
//                       // allow arbitrary metric fields:
//                       // reps, sets, rest_sec, time_min, time_sec,
//                       // bedtime, waketime, target_hours, calories, liters, etc.
...
//     "Blocks are free-form but must have { name, metrics } with metrics as a dictionary (reps, time_min, bedtime, calories, liters, etc.).",
...
//     throw new Error(`Generator returned ${parsed?.days?.length ?? 0} days, expected ${totalDays}.`);
//   }
//   if (!parsed?.metadata?.cadence_days_per_week) {
//     throw new Error("Missing cadence in metadata.");
//   }
//   return parsed; // { metadata, days }
// }
// universalProgramDays.ts
import { openai } from "./OpenaiClient";
...
function normalizeLength<T = any>(days: T[], target: number, planType: string): T[] {
