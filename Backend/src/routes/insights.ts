...
/**
 * GET /api/insights
 * Always generates fresh wellness insights based on recent important messages
 * Stores them in database and returns immediately
 */
router.get('/', async (_req, res) => {
  ...
  try {
    ...
    console.log('Fetching messages since:', thirtyDaysAgo.toISOString());
    console.log('Looking for HUMAN_ID:', HUMAN_ID, 'AI_ID:', AI_ID);
    ...
    console.log('Query result - messages count:', messages?.length);

    if (error) throw error;

    // If no important messages, return default insights
...
Return ONLY valid JSON.`;
    ...
    if (insertError) {
      ...
      // Don't throw error, still return insights even if storage failed
    } else {
      console.log('Insights saved to database successfully with ID:', savedInsights?.id);
    }
    ...
    // Fallback to latest stored insights on error
    try {
      const { data: fallbackInsights } = await supa
        .from('insights')
      ...
      if (fallbackInsights) {
        return res.json({
          insights: fallbackInsights.insights_data,
          generated: false,
          fallback: true,
          error: 'Used cached insights due to generation error'
        });
      }
    } catch (fallbackError) {
    ...
    // Final fallback to default insights
    res.status(500).json({
      insights: {
        observations: {
          cognition: "Share your focus, memory, and mental clarity to optimize cognitive performance.",
        ...
        nextActions: [
          {
            title: "Start Sharing",
            text: "Begin by sharing your current wellness experiences to get personalized insights."
          }
        ],
        reveal: "I'm here to help you discover patterns in your wellness journey. Share more about your daily experiences to unlock deeper insights."
      },
      generated: false,
      error: 'Failed to generate insights, showing default',
      details: e.message
    });
  }
});
